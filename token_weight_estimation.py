...
def calculate_probability_differences(model_1, model_2, tokenizer, prompts_1, prompts_2, responses, batch_size=8, device=None, process_id=None):
    ...
    for i in tqdm(range(0, len(prompts_1), batch_size), desc=desc, mininterval=1.0, ncols=80):
        ...
        batch_responses_raw = responses[i:i+batch_size]

        # --- Convert responses to list[str] ---
        batch_responses = []
        for r in batch_responses_raw:
            if isinstance(r, str):
                batch_responses.append(r)
            elif isinstance(r, list):
                assistant_messages = [msg for msg in r if msg.get("role") == "assistant"]
                if assistant_messages:
                    batch_responses.append(assistant_messages[-1]["content"])
                else:
                    batch_responses.append("")
            elif isinstance(r, dict) and "response" in r:
                batch_responses.append(str(r["response"]))
            else:
                batch_responses.append("")  # fallback nếu không phải string
        ...
        combined_input_ids_1, combined_attention_mask_1 = [], []
        combined_input_ids_2, combined_attention_mask_2 = [], []
        ...
        for j in range(len(batch_prompts_1)):
            ...
            # Concatenate
            combined_input_ids_1.append(torch.cat([prompt_ids_1, response_ids]))
            combined_attention_mask_1.append(torch.cat([prompt_mask_1, response_mask]))
            combined_input_ids_2.append(torch.cat([prompt_ids_2, response_ids]))
            combined_attention_mask_2.append(torch.cat([prompt_mask_2, response_mask]))

        # Pad sequences
        max_len_1 = max(len(ids) for ids in combined_input_ids_1)
        max_len_2 = max(len(ids) for ids in combined_input_ids_2)

        padded_input_ids_1 = [F.pad(ids, (0, max_len_1 - len(ids)), value=tokenizer.pad_token_id).long() for ids in combined_input_ids_1]
        padded_attention_mask_1 = [F.pad(mask, (0, max_len_1 - len(mask)), value=0).long() for mask in combined_attention_mask_1]
        padded_input_ids_2 = [F.pad(ids, (0, max_len_2 - len(ids)), value=tokenizer.pad_token_id).long() for ids in combined_input_ids_2]
        padded_attention_mask_2 = [F.pad(mask, (0, max_len_2 - len(mask)), value=0).long() for mask in combined_attention_mask_2]

        # Stack tensors và đưa lên device
        inputs_1 = {
        ...
        for j in range(len(batch_prompts_1)):
            prompt_len_1 = len(tokenizer.encode(batch_prompts_1[j])) - 1
            prompt_len_2 = len(tokenizer.encode(batch_prompts_2[j])) - 1
            response_len = len(tokenizer.encode(batch_responses[j], add_special_tokens=False))

            weights = []
            for k in range(response_len):
                token_id_1 = inputs_1['input_ids'][j, prompt_len_1 + k + 1].item()
                token_id_2 = inputs_2['input_ids'][j, prompt_len_2 + k + 1].item()

                assert token_id_1 == token_id_2, "Response tokens must match"

                score_1 = logits_1[j, prompt_len_1 + k, token_id_1]
                score_2 = logits_2[j, prompt_len_2 + k, token_id_2]
                weights.append(round(float(score_2 - score_1), 2))
...
def parallel_process_file(file_path, args):
    ...
    if shard_size == 0:
        shard_size = 1  # Ensure minimum shard size
