...
class Distiller(nn.Module):
    ...
    def load_existing_projectors(self):
        if self.config.projector_path is not None:
            ...
            projector_path = os.path.join(self.config.model.policy_name_or_path, "projector.pt")
    ...
    def load_student_model(self):
        ...
        config = AutoConfig.from_pretrained(self.config.model.policy_name_or_path, trust_remote_code=True)
        config.is_model_parallel = False

        tokenizer = self.load_tokenizer(self.config.model.model_type, self.config.model.policy_name_or_path)
        ...
        if self.config.model.policy_dtype == "float32":
            self.dtype = torch.float32
        elif self.config.model.policy_dtype == "bfloat16":
            self.dtype = torch.bfloat16
        elif self.config.model.policy_dtype == "fp16":
            self.dtype = torch.float16
        else:
            raise NotImplementedError(f"Invalid model_dtype for {self.config.model.policy_dtype}")
        
        model = AutoModelForCausalLM.from_pretrained(
            self.config.model.policy_name_or_path, 
            config=config, 
            device_map=None, 
    ...
    def load_teacher_model(self):
        ...
        config = AutoConfig.from_pretrained(self.config.model.reference_name_or_path)
        config.is_model_parallel = False

        tokenizer = self.load_tokenizer(self.config.model.teacher_model_type, self.config.model.reference_name_or_path)
        ...
        model = AutoModelForCausalLM.from_pretrained(
            self.config.model.reference_name_or_path, 
            config=config, 
            device_map=None, 
            torch_dtype=self.dtype,
            trust_remote_code=True
        ...
        return model, {self.config.model.teacher_model_type: tokenizer}
